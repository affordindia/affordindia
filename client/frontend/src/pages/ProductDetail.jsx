...
const ProductDetail = () => {
  ...
  if (!stock) {
    stockStatus = <div className="text-red-600 font-medium">Out of Stock</div>;
  } else if (stock > 10) {
    stockStatus = (
    ...
    stockStatus = (
      <div className="text-orange-500 font-medium">In Stock ({stock} left)</div>
    );
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-3 flex-1 min-w-0 text-[#404040]">
          ...
          {product.reviewsCount > 0 && (
            <div className="text-lg">
              ★ {product.ratings?.toFixed(1) || "0.0"} ({product.reviewsCount}{" "}
          ...
          <div className="flex gap-4 mt-4 flex-wrap">
            {isInCart || addingToCart ? (
              <Link
                to="/cart"
                className={`button ${addingToCart ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}
                tabIndex={addingToCart ? -1 : 0}
                aria-disabled={addingToCart}
                onClick={e => addingToCart && e.preventDefault()}
              >
                <span className="button-content montserrat-global">Go to Cart</span>
              </Link>
            ) : (
              <button
              ...
              >
                <span className="button-content montserrat-global">Add to Cart</span>
              </button>
            )}
            <button
          ...
          {wishlistMsg && (
            <p className="text-sm mt-1 text-gray-500">{wishlistMsg}</p>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-4 py-8">
        <ReviewsPreview productId={id} currentUserId={user?._id || user?.id} />
      </div>
