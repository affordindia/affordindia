...
const WishlistCard = ({ product }) => {
  ...
  return (
    <div className="bg-[#ffffff] rounded-xl shadow-md overflow-hidden relative hover:shadow-lg transition-all flex flex-col group w-full max-w-[280px]">
      {/* Product Link */}
      <Link to={`/products/id/${product._id}`} className="flex flex-col flex-1">
        {/* Image: reduced height from square to 180px */}
        <div className="w-full h-[180px] bg-gray-100 flex-shrink-0 overflow-hidden">
          <img
      ...
      {/* Action Buttons */}
      <div className="px-2 pb-2 pt-1 flex flex-row gap-2">
        {/* Move to Cart Button */}
        <button
          onClick={handleMoveToCart}
          disabled={loading || actionLoading || product.stock < 1}
          className="flex-1 bg-[#B76E79] text-white py-1 px-2 rounded-lg hover:bg-[#C68F98] disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-1 text-sm font-medium montserrat-global min-h-[32px]"
        ...
        <button
          ...
          className="w-9 xs:w-10 bg-white border border-[#8C8C8C] py-1 px-2 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center transition-colors duration-200 hover:bg-[#E35151] group/delete min-h-[32px] flex-shrink-0"
